        ...
        <?php if (!empty($hunting_items)): ?>
            ...
            <div class="stage-item-dashboard brand-item-dashboard" data-brand-id="<?= $item->id ?>" data-brand-name="<?= htmlspecialchars($item->name) ?>" data-stage="1">
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    ...
                    <!-- �9�7 TOMBOL EDIT DENGAN ONCLICK LANGSUNG -->
...
<div class="stage-card-dashboard" data-stage="2" style="display: flex; flex-direction: column; height: 500px; border-top: 3px solid #f59e0b;">
    ...
    <div id="followupItemsContainerDashboard" style="flex: 1; overflow-y: auto; padding: 0 8px;">
        <?php if (!empty($followup_items)): ?>
            ...
            // �9�7 TENTUKAN APAKAH BISA DIKLIK
    ...
    <div id="setupItemsContainerDashboard" class="stage-items-container">
        <?php if (!empty($setup_items)): ?>
            ...
            // �9�7 TOMBOL CEK STATUS (jika sudah submit dan tidak ada pending)
            $checkStatusBtn = '';
...
function sendWhatsAppDealDashboard(brandId, phoneNumber, message, bannerUrl, brandName, stage) {
    ...
    // �9�7 INSERT KE DATABASE WHATSAPP_LOGS
    const logData = {
        brand_id: brandId,
    ...
    if (bannerUrl) {
        setTimeout(() => {
            showToastInModal('�9�2 Jangan lupa kirim gambar banner secara terpisah!', 'info');
        }, 1000);
    }
}
...
async function showTask2FollowUpModal(brandId, brandName) {
    ...
    let maxCommission = result.max_commission || recommendedMax; // �9�7 MAX FLEKSIBEL
    
    // �9�7 SLIDER: MIN = recommendedMin, MAX = maxCommission (bisa diubah user)
    const sliderMin = recommendedMin;
    const sliderMax = Math.max(50, maxCommission + 10);
    
    // �9�7 CEK STATUS REGISTRASI BRAND
    ...
    // �9�7 BUILD CAMPAIGN OPTIONS HTML
    ...
    // �9�7 BUILD REGISTRATION STATUS HTML
    ...
    modalBodyElem.innerHTML = `
        ...
        <div style="background:#1a1f2e; border-radius:10px; padding:8px 12px; margin-bottom:16px;">
            ...
            <div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                <!-- �9�7 MIN: Rekomendasi Min (tidak bisa diubah) -->
                ...
                <!-- �9�7 MAX: Bisa diubah dengan slider -->
...
Terima kasih! </textarea>
    ...
    // �9�7 UPDATE PESAN BERDASARKAN CAMPAIGN & KOMISI
...
async function showTask3SetupModalWithRecommendations(brandId, brandName) {
    ...
    // �9�7 AMBIL DATA REQUIREMENT BRAND
    ...
    // �9�7 CEK APAKAH REQUIREMENT SUDAH TERISI
    ...
    // �9�7 BUILD REQUIREMENT FORM HTML
    ...
    // �9�7 JIKA SUPERVISOR DAN REQUIREMENT SUDAH DIISI
    ...
    // �9�7 JIKA BUKAN SUPERVISOR DAN REQUIREMENT BELUM DIISI
    else if (!isSupervisor && !hasRequirements) {
        requirementFormHtml = `
    ...
    // �9�7 JIKA SUPERVISOR TAPI REQUIREMENT BELUM DIISI
    else if (isSupervisor && !hasRequirements) {
        requirementFormHtml = `
    ...
    // �9�7 APPROVE SECTION (hanya untuk supervisor)
    let approveSection = '';
    let linkSection = '';
    ...
    // �9�7 RENDER MODAL
    // 🔥 JIKA DISABLE APPROVAL (NEED CLAIM), KUNCI FORM & ACTIONS
    if (disableApproval) {
        requirementFormHtml = '';
        requirementStatusHtml = '';
        linkSection = '';
...
if (approveResult.success) {
    ...
    // �9�7 CEK APAKAH SEMUA PRODUK SUDAH APPROVE �� PINDAH KE ACTIVE
    const remainingProducts = document.querySelectorAll('.product-card-landscape').length;
    
    if (remainingProducts === 0) {
        // �9�7 PANGGIL ENDPOINT UNTUK CEK DAN PINDAH KE ACTIVE
        try {
            ...
            if (moveResult.moved_to_active) {
                ...
                // �9�7 TUTUP MODAL DAN REFRESH HALAMAN
                setTimeout(() => {
                    closeModalDashboard();
                    location.reload();
                }, 1500);
    ...
    document.querySelectorAll('.reject-single-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (rejectResult.success) {
                showToastInModal(`�7�4 Produk ditolak!`, 'warning');
                const productCard = btn.closest('.product-card-landscape');
                if (productCard) productCard.remove();
            } else {
    ...
    // �9�7 APPROVE SECTION (batch) untuk supervisor
    if (isSupervisor && requirementsFilled) {
                    ...
                    if (linkResult.success) {
                        linksHtml += `
                            <div style="background:#0f1420; border-radius:8px; padding:10px; margin-bottom:10px;">
                                <div style="font-weight:500; margin-bottom:6px;">�9�4 ${escapeHtml(product.product_name)}</div>
...
function renderSearchResults(containerId, brands, searchEndpoint) {
    ...
    // �9�7 ATTACH EVENT LISTENER UNTUK TOMBOL EDIT (hasil search Task 1)
 container.innerHTML = html;
    ...
    // �9�7 ATTACH EVENT LISTENER UNTUK TOMBOL CHECK REGISTRATION (Task 2)
    ...
    // �9�7 ATTACH EVENT LISTENER UNTUK BRAND ITEMS
...
function displaySearchResultsInDropdown(results) {
    ...
    if (shops.length > 0) {
        shops.forEach(shop => {
            // �9�7 HITUNG RANGE KOMISI (min - max) dari semua produk
        ...
        newItem.addEventListener('click', async () => {
            ...
            if (type === 'shop') {
                ...
                if (commissionInfo && rangeCommission) {
                    ...
                    commissionInfo.innerHTML = `<i class="fas fa-chart-line"></i> �9�7 Total Komisi: ${highestCommission}% | Range: ${rangeCommission} | Total Produk: ${totalProducts || '0'}`;
                ...
                // �9�7 Simpan range komisi ke hidden input
    ...
    document.querySelectorAll('.search-result-item').forEach(item => {
        ...
        newItem.addEventListener('click', async () => {
            ...
            if (type === 'shop') {
                ...
                // �9�7 SIMPAN OPEN COMMISSION RATE (KOMISI TERTINGGI)
                if (commissionRateHidden && highestCommission) {
                    commissionRateHidden.value = highestCommission;
                    console.log('Open commission rate saved:', highestCommission);
                }
                
                // �9�7 TAMBAHKAN HIDDEN INPUT UNTUK OPEN COMMISSION RATE
...
function showNewBrandModal() {
    ...
    if (saveBtn) {
        ...
        newSaveBtn.addEventListener('click', async () => {
            ...
            // �9�7 PERUBAHAN: Validasi seller_id hanya jika user memilih dari hasil pencarian
...
document.addEventListener('DOMContentLoaded', () => {
    updateStageUIDashboard();
        ...
        // �9�7 SEARCH TASKS DENGAN API (mencari ke database)
...
// �9�7 EDIT BRAND BUTTON (di list item)
    ...
    try {
        ...
        if (result.success) {
            if (result.has_products) {
                // �9�7 BRAND SUDAH REGISTRASI - REFRESH HALAMAN
                showToastInModal('�7�3 Brand ' + brandName + ' sudah registrasi! Memindahkan ke Task 3...', 'success');
                setTimeout(() => location.reload(), 1500);
            } else {
                // Belum registrasi
...
document.addEventListener('click', async function(e) {
    ...
    if (successCount > 0) {
        ...
        showToastInModal(`�7�4 Gagal menolak ${failCount} produk`, 'error');
    }
    ...
    // �9�7 CEK APAKAH SEMUA PRODUK SUDAH DIPROSES (APPROVE ATAU REJECT)
    if (remainingProducts === 0) {
        try {
            const moveResponse = await fetch(baseUrlDashboard + 'bd/check_and_move_to_active', {
                method: 'POST',
